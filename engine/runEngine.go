...
func GetBookInfos(url string) {
	...
	for _, m := range matches {
		// string转int8
		level, _ := strconv.ParseInt(m[3], 10, 64)
		// string转float
		size, _ := strconv.ParseFloat(m[2], 32)
		// fmt.Printf("Author:%v, BookName:%v, Size:%TMB, Level:%T, Date:%v\n",m[1],m[9],m[2],m[3],m[4])
		err := mysql.InsertInfo(m[1], m[9], m[4], size, level)
		common.HandlerError(err, "[error] InsertInfo failed")
		// fmt.Printf("Author:%v, BookName:%v, Size:%TMB, Level:%T, Date:%v\n",m[1],m[9],size,level,m[4])
	}
}
